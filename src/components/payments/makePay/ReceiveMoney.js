...
export const ReceiveMoney = () => {
    ...
    const handleInputChange = (e) => {
        if (parseInt(e.key) >= 0) {
            ...
            setShowInput(deleteNumber.length > 0 ? `$${deleteNumber}.00 (${deleteNumber === '1' ? 'un peso' : `${numberToText(deleteNumber)} pesos`})` : "$")
        }
    }
    ...
    return (
        <form className={`make__formMoney ${!totalPayMoney && 'ui__disabledEffect inputsPay'}`}>
            ...
            <div className="make__containerInput">
                ...
                <input className="make__money" type="text" onChange={() => { }} value={totalPayMoney === "" ? "" : `$${totalPayMoney - inputNumbers} (${(totalPayMoney - inputNumbers) === 1 ? "un peso" : `${numberToText(totalPayMoney - inputNumbers)} pesos`})`} />
            </div>
        </form>
    )
}
